...
# from utility_func import create_sql_engine, create_kafka_producer, logger_function
from itertools import product
from tqdm.auto import trange
from tqdm import tqdm  # Remove when using Airflow
from sqlalchemy import create_engine  # Remove when using Airflow
import datetime
import logging  # Remove when using Airflow
...
from tabulate import tabulate  # Remove when using Airflow
from sqlalchemy.exc import ProgrammingError
from requests.exceptions import RequestException


class TqdmLoggingHandler(logging.Handler):  # Remove when using Airflow
...
class Scraper:
    ...
    def __init__(self):
        ...
        self.towns = []
        self.session = None
        self.engine = create_engine("postgresql+psycopg2://postgres:Xy14RNw02SmD@database-1.chuq28s6itob.us-east-2.rds.amazonaws.com:5432/nj_realtor_data")
    ...
    @staticmethod
    def logger_decorator(original_function):  # Remove when using Airflow
        ...
        with self.session.get(kwargs['url'], params=kwargs['params'], stream=True) as reader:
            try:
                ...
                if 'PDF' in check_pdf:
                    ...
                    # value = response.content key = kwargs['key']
    ...
    def event_log_update(self, logger):
        ...
        if not pd.read_sql_table(table_name, self.engine).empty:  # Use engine.raw_connection() with Airflow
            # db.to_sql(table_name, self.engine, if_exists='append', index=False)  # Use engine.raw_connection() with Airflow
            print(tabulate(db, headers=db.columns))
